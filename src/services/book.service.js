...
const gBooks = [
    ...
    },
    {
        ...
        "publishedDate": 2011,
        "description": "aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed aliquet euismod mi vivamus bibendum donec etiam quisque iaculis ullamcorper est sed",
        "pageCount": 904,
        "categories": [
            "Computers",
            "Hack"
...
async function save(book) {
    if (book.coverUploaded) {
        book.thumbnail = await cloudinaryService.upload(book.coverUploaded);
    }
...
function formatGoogleBook(gbook) {
    var newBook = getEmptyBook();
    newBook.title = gbook.volumeInfo.title || '';
    newBook.subtitle = gbook.volumeInfo.subtitle || '';
    newBook.thumbnail = gbook.volumeInfo.imageLinks.thumbnail || '';
    newBook.authors = gbook.volumeInfo.authors || [];
    newBook.publishedDate = gbook.volumeInfo.publishedDate || '';
    newBook.description = gbook.volumeInfo.description || '';
    newBook.pageCount = gbook.volumeInfo.pageCount || null;
    newBook.categories = gbook.volumeInfo.categories || [];
    newBook.language = gbook.volumeInfo.language || '';
    newBook.listPrice = {
        amount: 20,
        currencyCode: "EUR",
        isOnSale: false,
...
async function getPrevNext(id){
    var books = await dbService.query(KEY);
    var currIndex = books.findIndex(book=>book.id === id);    
    const prevInd = currIndex === 0 ? 0 : currIndex - 1;
    const nextInd = currIndex === books.length - 1 ? books.length - 1 : currIndex + 1;    
    const prevNext = { prev: books[prevInd].id, next: books[nextInd].id };
    return Promise.resolve(prevNext);
}
